...
def all_cli(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(description="Update the observational archives used by floes in one copyq job.")
    p.add_argument("--dest-root", type=Path, default=Path("/g/data/gv90/da1339/SeaIce"))
    p.add_argument("--history-years", type=int, default=3)
    p.add_argument("--esa-lookback-years", type=int, default=5, help="Existing SeaIce archive root on Gadi.")
    ...
    p.add_argument("--history-years", type=int, default=3)
    p.add_argument("--esa-lookback-years", type=int, default=5)
    ...
    jobs.extend(build_nsidc_g02202_jobs(dest_root=args.dest_root / "NSIDC", hemis=["north", "south"], start_year=args.start_year, end_year=args.year, daily_mode="aggregate", monthly_mode="none", include_ancillary=True))
    ...
    recent = set(_recent_months(args.year, args.month, args.bremen_lookback_months))
    history = set()
    for offset in range(args.history_years):
        y, m = args.year - offset, args.month - 1
        if m == 0:
            y -= 1
            m = 12
        history.update(_recent_months(y, m, 3))
    requested_bremen = recent | history
    for year in sorted({y for y, _ in requested_bremen}):
        months = sorted(m for y, m in requested_bremen if y == year)
        jobs.extend(build_bremen_amsr2_jobs(dest_root=args.dest_root, year=year, months=months, start_year=args.year - args.esa_lookback_years + 1, end_year=args.year))
    # ESA CCI SIT:
    # Search a modest backward window so the newest remotely available year is
    # captured even when CCI publication lags the current calendar year, while
    # retaining enough history for the rolling 3 x 3 gallery table.
    jobs.extend(build_esa_cci_sit_jobs(dest_root=args.dest_root, levels=("L2P", "L3C"), sensors=("envisat", "cryosat2", "sentinel3a", "sentinel3b"), hemisphere="SH"))
