...
def write_gallery(*, fig_dir: Path, md_path: Path, title: str = "Monthly Sea Ice Science Chat Figures") -> Path:
    ...
    lines = [f"# {title}", "", f"Last refreshed: {datetime.now().isoformat(timespec='seconds')}", "", f"Figure directory: `{fig_dir}`", ""]
    if not figures:
        lines.extend(["No figures were found yet.", "", "Run:", "", "```bash", "qsub ./update_mthly_sea_ice_sci_chat_figs.pbs", "```", ""])
    else:
        for fig in figures:
