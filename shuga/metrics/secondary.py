...
def compute_seasonal_summary_dataset(*, requested: set[str], dispatcher, output: xr.Dataset, seasonal_requests: dict[str, set[str]],
                                     compute_seasonal_summary: Callable[[xr.DataArray, str], dict[str, xr.DataArray]]) -> xr.Dataset:
    """
...
def compute_fipsi_dataset(*, requested: set[str], fipsi_names: set[str], fi_mask: xr.DataArray | None,  area: xr.DataArray,
                          persistence_stability_index: Callable[[xr.DataArray, xr.DataArray], dict[str, xr.DataArray]]) -> xr.Dataset:
    """
...
def compute_obs_skill_dataset(*, requested: set[str], dispatcher, output: xr.Dataset, fia_skill_names: set[str], fit_skill_names: set[str],
                              obs_skill_dataset: Callable[[xr.Dataset], xr.Dataset]) -> xr.Dataset:
    """
...
def common_metrics_attrs(*, sim_name: str, start_date: str, end_date: str, hemisphere: str, ice_type: str, grid_type: str, method: str) -> dict[str, str]:
    """
...
def attach_common_metrics_attrs(ds: xr.Dataset, *,
                                sim_name  : str,
                                start_date: str,
                                end_date  : str,
                                hemisphere: str,
    ...
    """
    ds = ds.copy()
    ds.attrs.update(common_metrics_attrs(sim_name   = sim_name,
                                         start_date = start_date,
                                         end_date   = end_date,
                                         hemisphere = hemisphere,
                                         ice_type   = ice_type,
