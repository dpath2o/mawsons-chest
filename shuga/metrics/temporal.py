...
def month_window_bounds(year_start: int, start_month: int, end_month: int) -> tuple[pd.Timestamp, pd.Timestamp]:
    """
...
def seasonal_rate_record(series             : pd.Series, *,
                         year_start         : int,
                         start_month        : int,
                         end_month          : int,
                         prefix             : str,
    ...
    """
    window_start, window_end = month_window_bounds(year_start = year_start, start_month = start_month, end_month = end_month)
...
def compute_extrema_table(da: xr.DataArray, *,
    ...
    if mode in {"calendar", "cal", "year"}:
        ...
        period_bounds = {int(y): month_window_bounds(int(y), 1, 12) for y in np.unique(year_start)}
        ...
        period_bounds = {int(y): month_window_bounds(int(y), 3, 2) for y in np.unique(year_start)}
    else:
    ...
    for period, grp in grouped:
        ...
        if growth_window is not None:
            row.update(seasonal_rate_record(series,
                                            year_start=ys,
