...
def _open_grouped_iceh_store(
    ...
    *,
    ...
    logger.info(
        "Opening grouped %s CICE Zarr between %s and %s (%d groups)",
        group_label,
        user_dt0,
        user_dtN,
        len(required_groups),
    ...
    def load(
        ...
        *,
        ...
        if variables_list is None:
            self.logger.warning(
                "IceHistoryLoader.load() called with variables=None; this may be expensive."
            )

        zarr_root = (
            Path(cice_store).expanduser()
            if cice_store is not None
            else self.paths.resolve_cice_store()
        )

        static_eff = (
            Path(static_store).expanduser()
            if static_store is not None
            else self.paths.resolve_static_store()
        )
        ...
        if variables_list is not None:
            ...
            if not present:
                raise ValueError(
                    f"None of the requested variables were found after merging "
                    f"static/dynamic stores: {variables_list}"
                )
