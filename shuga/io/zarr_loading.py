...
def _resolve_run_context(
    ...
    *,
    ...
    user: str | None = None,
) -> tuple[RunSpec, str | None, str | None, str | None]:
...
def _resolve_store_with_method_candidates(
    *,
    ...
    if not matches:
        ...
        raise FileNotFoundError(
            f"Could not resolve any {store_kind} store for sim={sim_name!r}. "
            f"Tried methods: {method_list}."
            + (f"\nRecent lookup failures:\n{details}" if details else "")
        )
    match_lines = "\n".join(
        f"- method={m.method}, grid_type={m.grid_type}, path={m.path}"
        for m in matches
    )
    raise ValueError(
        f"Multiple candidate {store_kind} stores found for sim={sim_name!r}. "
...
def load_cice(
    ...
    *,
    ...
    if variables_list is None:
        LOGGER.warning(
            "load_cice() called with variables=None; this can be expensive for long periods."
        )
...
def load_classified(
    ...
    *,
    ...
    if "FI_mask" not in ds.data_vars:
        raise KeyError(
            f"Could not find FI_mask in {resolved.path}. "
    ...
    if variables_list is not None:
        ...
        if not keep:
            raise ValueError(
                f"None of the requested classified variables were found in "
