...
# class ClassNameFilter(logging.Filter):
#     """
#     Ensure custom logging fields used by the formatter always exist.
#     """
...
# def set_default_logger(logger: logging.Logger | logging.LoggerAdapter | None) -> None:
#     """
#     Register the process-wide default Shuga logger.
#     """
#     global _DEFAULT_SHUGA_LOGGER
#     if logger is None:
#         _DEFAULT_SHUGA_LOGGER = None
...
# def get_default_logger() -> logging.Logger | None:
#     """
#     Return the currently registered default Shuga logger, if any.
#     """
#     return _DEFAULT_SHUGA_LOGGER
...
# ) -> logging.Logger | logging.LoggerAdapter:
#     """
#     Resolve a usable logger for free functions and class methods.

#     Precedence:
#     1. explicitly supplied logger
#     2. globally registered default logger
#     3. standard library logger by name
#     """
#     if logger is not None:
#         return logger
...
#     # notebook-safe: rebuild handlers
#     if logger.handlers:
#         for h in list(logger.handlers):
#             logger.removeHandler(h)
#             try:
...
#     fmt = "%(asctime)s - %(levelname)s - [%(module)s.%(cls_prefix)s%(funcName)s:%(lineno)d] %(message)s"
#     formatter = logging.Formatter(fmt)
#     class_filter = ClassNameFilter()
...
#     # Register as current Shuga default logger
#     set_default_logger(logger)
