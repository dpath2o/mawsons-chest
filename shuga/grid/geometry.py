...
def latlon_to_degrees(
    ...
    *,
    ...
    if "radian" in units or (
        finite.any() and np.nanmax(np.abs(values[finite])) <= 2.0 * np.pi + 1e-6
    ):
    ...
    attrs.setdefault(
        "standard_name",
        "longitude" if target.upper().endswith("LON") else "latitude",
    )

    return xr.DataArray(
        values,
        dims=da.dims,
        coords=dim_coords(da),
        attrs=attrs,
        name=target,
    )
