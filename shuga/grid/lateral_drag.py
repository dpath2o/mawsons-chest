...
class FormFactors:
    ...
    def _empty_f2_dataset(tlon, tlat, f2x, f2y, attrs: dict[str, object]) -> xr.Dataset:
        ds = xr.Dataset(
            data_vars={
                "F2x": (("nj", "ni"), f2x.astype("float32")),
                "F2y": (("nj", "ni"), f2y.astype("float32")),
                "lon": (("nj", "ni"), tlon.astype("float32")),
                "lat": (("nj", "ni"), tlat.astype("float32")),
            },
            coords={"nj": np.arange(tlon.shape[0], dtype=np.int32), "ni": np.arange(tlon.shape[1], dtype=np.int32)},
            attrs=attrs,
        )
    ...
    def build_F2_from_high_res_coastline(
        self,
        ...
        ds_out = self._empty_f2_dataset(
            tlon,
            tlat,
            f2x,
            f2y,
            attrs={
                "title": "Coastline-derived lateral drag form factors",
        ...
        enc = {
            "F2x": {"zlib": True, "complevel": int(self.spec.netcdf_compression), "dtype": "float32"},
    ...
    def build_F2_from_grounded_iceberg_dataframe(
        self,
        ...
        ds_out = self._empty_f2_dataset(
            ...
            f2y,
            attrs={
                "title": "Grounded-iceberg-derived lateral drag form factors",
    ...
    def build_F2_combined(
        ...
        ds_out = self._empty_f2_dataset(
            ...
            f2y,
            attrs={
                "title": "Combined coastline + grounded iceberg lateral drag form factors",
