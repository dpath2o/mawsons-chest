...
class CICEStaticBuilder:
    ...
    def resolve_run_metadata_file(self) -> Path | None:
        ...
        try:
            ice_diag = self.paths.resolve_ice_diag_file()
        except Exception as exc:
        ...
        for candidate in (
            self.paths.output_root / "ice_diag.d",
    ...
    def build_dataset_from_resolved_assets(
        self,
        *,
        ...
        if metadata_file is None and require_metadata:
            self.logger.warning(
                "Cannot build static CICE dataset because neither ice_in nor "
                "ice_diag.d was found."
            )
            return None
        ...
        if grid_file is None or not Path(grid_file).expanduser().exists():
            self.logger.warning(
                "Cannot build static CICE dataset: resolved grid_file is missing: %s",
                grid_file,
            )
            return None

        return self.build_dataset_from_grid_assets(
            assets=assets,
            metadata_file=metadata_file,
        )

    def write_zarr_from_resolved_assets(
        self,
        *,
        ...
        target = (
            ...
            else self.paths.resolve_static_store_target()
        )
        ...
        ds_static = self.build_dataset_from_resolved_assets(
    ...
    def build_dataset_from_grid_assets(
        self,
        *,
        ...
        bathymetry_file = (
            Path(assets["bathymetry_file"]).expanduser()
            if assets.get("bathymetry_file") is not None
            else None
        )

        gridwork = CICEGridwork(self.paths, logger=self.logger)
        bundle = gridwork.load_cice_grid(
            P_grid=grid_file,
            P_mask_org=kmt_file,
            build_faces=True,
        )
            ...
            for target in ("ULON", "ULAT"):
                if target in bundle.ugrid and target not in ds_static:
                    ds_static = ds_static.assign_coords(
                        {target: self.prepare_da(bundle.ugrid[target], name=target, context="grid-faces")}
                    )
        ...
        if bundle.egrid is not None:
            for target in ("ELON", "ELAT"):
                if target in bundle.egrid and target not in ds_static:
                    ds_static = ds_static.assign_coords(
                        {target: self.prepare_da(bundle.egrid[target], name=target, context="grid-faces")}
                    )
        ...
        if bundle.ngrid is not None:
            for target in ("NLON", "NLAT"):
                if target in bundle.ngrid and target not in ds_static:
                    ds_static = ds_static.assign_coords(
                        {target: self.prepare_da(bundle.ngrid[target], name=target, context="grid-faces")}
                    )
    ...
    def fill_missing_static_geometry_from_tgrid(self, ds_static: xr.Dataset) -> xr.Dataset:
            ...
            for name in ("uarea", "earea", "narea"):
                if name not in ds:
                    ...
                    ds[name].attrs.update(
                        {
                            "long_name": f"{name} fallback copied from tarea",
        ...
        for target, source in metric_fallbacks.items():
            if target not in ds and source in ds:
                ...
                ds[target].attrs.update(
                    {
                        "long_name": f"{target} fallback copied from {source}",
    ...
    def fill_missing_masks_from_tmask(self, ds_static: xr.Dataset) -> xr.Dataset:
        ...
        for name, mask in derived.items():
            ...
            ds[name].attrs.update(
                {
                    "long_name": f"{name} derived conservatively from tmask",
                    "units": "1",
                }
            )
