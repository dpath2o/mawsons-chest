...
class CICEGridwork:
    ...
    def build_grid_faces(self, lon, lat, source_in_radians: bool = False):
        ...
        lon_b[1:-1, 1:-1] = self._circular_mean_lon_deg(
    ...
    def load_cice_grid(
        ...
        tlon, tlat, anglet_rad, dx_m, dy_m, area_m2, grid_kind, ds_grid = self.load_super_grid(
            P_grid=P_grid, lon_type=self.grid_spec.lon_type, nx=nx, ny=ny
        )
        ...
        tgrid = xr.Dataset(
            data_vars={
                "TLON": (("nj", "ni"), np.asarray(tlon, dtype="float64")),
            ...
            coords={"nj": np.arange(nj, dtype=np.int32), "ni": np.arange(ni, dtype=np.int32)},
            attrs={"grid_kind": grid_kind, "source_path": str(P_grid or self.paths.cice_grid_path)},
        )
        ...
        if build_faces:
            ...
            ugrid = xr.Dataset(
                data_vars={
                    "ULON": (("nj_b", "ni_b"), lon_b),
                    "ULAT": (("nj_b", "ni_b"), lat_b),
                },
                coords={"nj_b": np.arange(lon_b.shape[0], dtype=np.int32), "ni_b": np.arange(lon_b.shape[1], dtype=np.int32)},
            )
            egrid = xr.Dataset(
                data_vars={"ELON": (("nj", "ni_b"), lon_e), "ELAT": (("nj", "ni_b"), lat_e)},
                coords={"nj": np.arange(lon_e.shape[0], dtype=np.int32), "ni_b": np.arange(lon_e.shape[1], dtype=np.int32)},
            )
            ngrid = xr.Dataset(
                data_vars={"NLON": (("nj_b", "ni"), lon_n), "NLAT": (("nj_b", "ni"), lat_n)},
                coords={"nj_b": np.arange(lon_n.shape[0], dtype=np.int32), "ni": np.arange(lon_n.shape[1], dtype=np.int32)},
            )

        bundle = CICEGridBundle(
            tgrid=tgrid,
            ugrid=ugrid,
            egrid=egrid,
            ngrid=ngrid,
            ...
            metadata={
                "ice_in_file": str(self.paths.resolve_cice_grid_assets().get("ice_in_file")) if self.paths.resolve_cice_grid_assets().get("ice_in_file") is not None else None,
                "grid_file": str(self.paths.cice_grid_path),
                "kmt_file": str(self.paths.cice_kmt_path),
                "bathymetry_file": str(self.paths.cice_bathymetry_path) if self.paths.cice_bathymetry_path is not None else None,
                "f2_file": str(self.paths.cice_f2_path) if self.paths.cice_f2_path is not None else None,
            },
        )
