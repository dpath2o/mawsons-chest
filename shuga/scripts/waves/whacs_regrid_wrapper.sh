...
Gadi accepts -J X-Y[:Z]; it does not use the Slurm-style %N concurrency suffix.
...
Options:
  ...
  --ow_sic_wgt              rebuild NSIDC weights
...
VARS="START_YEAR=${START_YEAR},END_YEAR=${END_YEAR},ACCOUNT_PROJECT=${ACCOUNT_PROJECT},DATA_PROJECT=${DATA_PROJECT},RUN_USER=${RUN_USER},SIM_NAME=${SIM_NAME},TARGET_LAT_MAX=${TARGET_LAT_MAX},SIC_THRESHOLD=${SIC_THRESHOLD},K_NEAREST=${K_NEAREST},IDW_POWER=${IDW_POWER},RADIUS_KM=${RADIUS_KM},TIME_CHUNK=${TIME_CHUNK},COMPRESSION_LEVEL=${COMPRESSION_LEVEL},OW_NC=${OW_NC},OW_WGT=${OW_WGT},OW_SIC_WGT=${OW_SIC_WGT}"
