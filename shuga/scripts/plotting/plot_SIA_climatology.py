...
REFERENCE_COLORS    = {"CMEMS": "#61D97B"}
MODEL_COLORS        = {"no-slip-LFI": "#D55E00",
                       "Cs-high"    : "#E69F00",
                       "Cq-high"    : "#FF99CC"}
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description = ("Plot SIA climatology for standalone CICE experiments against "
                                               "NSIDC, OSI-SAF-450 and CMEMS."))
    p.add_argument("--start-date", required = True)
    p.add_argument("--end-date", required = True)
    p.add_argument("--paper-root", type = Path, required = True, help = ("Root containing experiment directories, e.g. "
                                                                         "/g/data/gv90/da1339/afim_output/paper1 or "
                                                                         "/g/data/gv90/da1339/afim_output"))
    p.add_argument("--experiments", default = None, help = ("Comma-separated SIM=LABEL entries. If omitted, defaults to no-slip-LFI,Cs-high,Cq-high."))
    ...
    p.add_argument("--cmems-store", type = Path, default = Path("/g/data/gv90/da1339/SeaIce/CMEMS/0p083/daily/SH/SI/mets.zarr"))
...
def main() -> None:
    ...
    pth_cfg      = ShugaPaths(run_cfg = run_cfg, cls_cfg = cls_cfg, obs_cfg = obs_cfg, afim_output_root = args.paper_root, graphics_root = args.output.parent)
    ...
    series["NSIDC"] = plotter.load_sia_store(nsidc_store,
                                             label      = "NSIDC",
                                             start_date = args.start_date,
                                             end_date   = args.end_date,
                                             variable   = "SIA")
    series["OSI-SAF-450"] = plotter.load_sia_store(osisaf_store,
                                                   label      = "OSI-SAF-450",
                                                   start_date = args.start_date,
                                                   end_date   = args.end_date,
                                                   variable   = "sia")
    series["ORAS"] = plotter.load_sia_store(args.cmems_store,
                                             label      = "CMEMS",
                                             start_date = args.start_date,
                                             end_date   = args.end_date,
                                             variable   = "SIA")
    for sim_name, label in experiments.items():
        ...
        sim_paths     = ShugaPaths(run_cfg = sim_run, cls_cfg = sim_cls, afim_output_root = args.paper_root)
        store         = sim_paths.output_root / "zarr" / "SH" / "SI" / "mets.zarr"
        series[label] = plotter.load_sia_store(store,
                                               label=label,
                                               start_date=args.start_date,
                                               end_date=args.end_date,
                                               variable="SIA")
    ...
    order        = [name for name in (REFERENCE_ORDER + model_labels) if name in df.columns]
    model_colors = {label: MODEL_PALETTE[i % len(MODEL_PALETTE)] for i, label in enumerate(model_labels)}
