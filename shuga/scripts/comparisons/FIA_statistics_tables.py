...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description = ("Build AF2020/simulation FIA statistics and simulation-minus-AF2020 "
...
def summarise_fia(da: xr.DataArray, *, source: str, sim_name: str, start_date: str, end_date: str,
    ...
    for region in [str(v) for v in da["region"].values]:
        ...
        values = {"mean": _safe_float(series.mean(skipna=True).values) if n_time else np.nan,
...
def seasonal_rate_statistics(da: xr.DataArray, *, source: str, sim_name: str, start_date: str, end_date: str,
    ...
    for region in [str(v) for v in da["region"].values]:
        ...
        for season, months in season_months.items():
            ...
            values = {"mean": float(np.nanmean(slopes)) if slopes.size else np.nan,
...
def difference_table(df: pd.DataFrame, *, join_cols: list[str]) -> pd.DataFrame:
    ...
    merged["diff_pct_af2020"] = np.where(np.isfinite(merged["af2020_value"]) & (merged["af2020_value"] != 0.0),
                                         100.0 * merged["diff"] / np.abs(merged["af2020_value"]), np.nan)
...
def main() -> None:
    ...
    stats_frames = [summarise_fia(af_fia,
                                  source             = "observation",
                                  sim_name           = "AF2020",
                                  start_date         = args.start_date,
                                  end_date           = args.end_date,
                                  min_edge_trim_days = 0)]
    rate_frames  = [seasonal_rate_statistics(af_fia,
                                            source          = "observation",
                                            sim_name        = "AF2020",
                                            start_date      = args.start_date,
                                            end_date        = args.end_date,
    ...
    for sim_name in sim_names:
        ...
        stats_frames.append(summarise_fia(sim_fia,
                                          source             = "simulation",
                                          sim_name           = sim_name,
                                          start_date         = args.start_date,
                                          end_date           = args.end_date,
                                          min_edge_trim_days = min_edge_trim_days))
        rate_frames.append(seasonal_rate_statistics(sim_fia,
                                                    source          = "simulation",
                                                    sim_name        = sim_name,
                                                    start_date      = args.start_date,
                                                    end_date        = args.end_date,
    ...
    outputs    = {"stats": table_root / f"FIA_statistics_{tag}.csv",
